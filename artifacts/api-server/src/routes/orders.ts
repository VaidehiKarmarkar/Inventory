...
router.get("/orders/:id/invoice", requireAuth, async (req, res): Promise<void> => {
  ...
  doc.rect(40, 40, 515, 80).fill("#fef9c3");

  // "INVOICE" centered at top of banner
  doc.fillColor("#000000").fontSize(14).font("Helvetica-Bold").text("INVOICE", 40, 47, { width: 515, align: "center" });

  const createdDateStr = new Date(order.createdAt).toLocaleDateString("en-IN", {
    day: "2-digit", month: "short", year: "numeric"
  });

  // Second row: Store name (left) | Invoice Number (right)
  doc.fontSize(11).font("Helvetica-Bold").fillColor("#000000").text("Aloha Crystal World, Amravati", 55, 64);
  doc.fontSize(10).font("Helvetica-Bold").text(order.invoiceNumber, 360, 64, { width: 180, align: "right" });

  // Third row: Address (left) | Date (right)
  doc.fontSize(8).font("Helvetica").fillColor("#444444").text("Vedanta Heights, Shri Colony, Dastur Nagar, Amravati. | Mob: 8369495476", 55, 80, { width: 250 });
  doc.fontSize(8).font("Helvetica").fillColor("#444444").text(`Date: ${createdDateStr}`, 360, 80, { width: 180, align: "right" });
